...
public function printHead(){
    ...
    require('includes/config.inc.php');
    //Заголовок
    //начало исправления бага #337 
    // http://www.instantcms.ru/bugtraq/52/bug/337
    //очищаем от HTML тегов
    $title = preg_replace("~<.*?>~","",$this->title);
    if ($inCore->menuId() != 1) {
        echo '<title>'.$title.' - '.$_CFG['sitename'].'</title>' ."\n";
    //окончание исправления бага #337
    } else {
        echo '<title>'.$this->homeTitle().'</title>' ."\n";
    }
    //Ключевые слова
    if (!$this->page_keys) { $this->page_keys = $_CFG['keywords']; }
    echo '<meta name="keywords" content="'.$this->page_keys.'" />' ."\n";
    //Описание
    if (!$this->page_desc) { $this->page_desc = $_CFG['metadesc']; }
    echo '<meta name="description" content="'.$this->page_desc.'" />' ."\n";
    //Генератор
    echo '<meta name="generator" content="InstantCMS - www.instantcms.ru"/>' ."\n";
    //Плавная смена страниц для IE
    if (@$_CFG['page_fx']){
    ...
    //jQuery treeview
    echo '<script type="text/javascript" src="/includes/jquery/treeview/jquery.treeview.js"></script>' ."\n";
    echo '<script type="text/javascript" src="/includes/jquery/treeview/menu.js"></script>' ."\n";
    //Остальные JS-файлы
    ...
    echo '<link href="/includes/jquery/treeview/jquery.treeview.css" rel="stylesheet" type="text/css" />' . "\n";
    //CSS-файлы
    foreach($this->page_head as $key=>$value) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * Выводит на экран шаблон
 * @global array $_CFG
 * @global int $menuid
 * @param string $template_dir
 */
public function showTemplate($template_dir){
    $inCore = cmsCore::getInstance();
    $inUser = cmsUser::getInstance();
    $inPage = cmsPage::getInstance();
    global $_CFG;
    global $menuid;
    if ($template = $inCore->menuTemplate($menuid)){
...
public function showSplash(){
    $inCore = cmsCore::getInstance();
    $sfile = $_SERVER['DOCUMENT_ROOT'].'/core/splash/splash.php';
...
public static function getBBCodeToolbar($field_id, $images=0, $placekind='forum'){
    ...
    $html = '<a class="usr_bb_button" href="javascript:addTag(\''.$field_id.'\', \'[b]\', \'[/b]\')" title="Жирный">
             ...
             <a class="usr_bb_button" href="javascript:addTag(\''.$field_id.'\', \'[code]\', \'[/code]\')" title="Вставить код">
...
public function siteOffNotify() {
    return '<div style="margin:0px; padding:5px; border-bottom:2px solid gray; background:#FFF;">
                <strong style="color:red">Сайт отключен.</strong> Только администраторы видят его содержимое. Вы можете <a href="/admin/index.php?view=config">включить сайт</a> в панели управления.
            </div>';
}
