...
function pageBar($total, $current, $perpage){
    $inCore = cmsCore::getInstance();
    $inDB = cmsDatabase::getInstance();
    $html = '';
    if ($total){
...
function tagLine($tagstr, $cat_id){
    ...
    foreach($tags as $key=>$value){
        ...
        $html .= '<a href="/catalog/'.$inCore->menuId().'/'.$cat_id.'/tag/'.urlencode($value).'">'.$value.'</a>';
...
function catalog(){
    ...
    $inPage->addHeadCSS('components/catalog/css/styles.css');
        ...
        if (isset($_POST['gosearch'])){
            ...
            if (is_array($fdata)){
                ...
                $findsql = "SELECT i.* , IFNULL(AVG(r.points),0) AS rating
                            ...
                            WHERE i.published = 1 AND i.category_id = $id AND i.fieldsdata LIKE '$fstr' AND i.title LIKE '%$title%'";
    ...
    if ($do == 'cat'){
        ...
        if ($inDB->num_rows($catres)>0){
            ...
            if ($itemscount>0){
                ...
                $smarty->assign('pagebar', pageBar($itemscount, $page, $perpage));
            }
