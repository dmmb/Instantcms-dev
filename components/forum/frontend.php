...
function forum(){
	...
	$inPage->addHeadCSS('components/forum/css/styles.css');
				...
				while ($p = $inDB->fetch_assoc($presult)){
					...
					echo '<tr>';
						...
						echo '<td width="" class="post_msgcell" align="left" valign="top">';
							...
							echo '<td><strong>#'.$num.'</strong> - '.forumDate($p['pubdate'], $p['daysleft']) . ' в ' .$p['pubtime'].'</td>';
							echo '<td align="right">';
	...
	if (usrCheckAuth()){
		...
		if(!isset($_POST['gosend'])){		
			...
			if($do=='newpost'){												
				...
				$file_error = false;
				if ($cfg['fa_on'] && sizeof($_FILES['fa'])>0) {									
				...
				if ($do=='newthread'){
					...
					if($title && $message){	
						...
						if ($poll_error) { echo $poll_error; } else {
							//if poll created without errors
							//upload attached files
							$file_error = false;	
							if ($cfg['fa_on'] && sizeof($_FILES)>0) {							
								//get post id
								$sql = "SELECT LAST_INSERT_ID() as post_id FROM cms_forum_posts";
								$res = $inDB->query($sql) ;
								$postid = $inDB->fetch_assoc($res);				
								$post_id = $postid['post_id'];
								//upload
								$file_error = uploadFiles($post_id, $cfg);		
							}			
							if (!$file_error){
