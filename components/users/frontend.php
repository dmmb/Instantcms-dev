...
if ($do=='profile'){
    ...
    $usr_auth = $inConf ->user_view_prof;
    if ( cmsUserId() == 0 && $cfg['sw_guest_browse_profiles'] == 0 && $usr_auth == 1) {
        echo '<p>'.$_LANG['NOT_USER_VIEW_PROFILE'].'</p>';
        return;
    }
                                        ...
                                        <td width="460" align="right" valign="top"><table width="460">
                                          ...
                                          <tr>
                                            <td valign="top"><input name="tags" type="text" id="tags" size="40" value="<?php if (isset($photo['id'])) { echo cmsTagLine('userphoto', $photo['id'], false); } ?>"/>
                                              <br />
                                              <span><small><? echo $_LANG['KEYWORDS']; ?></small></span></td>
                                          </tr>
                                          <tr>
                                            <td valign="top"><strong><? echo $_LANG['REPLACE_FILE']; ?></strong>:</td>
                                          </tr>
                                          <tr>
                                          ...
                                          <tr>
                                            <td valign="top"><strong><? echo $_LANG['SHOW']; ?>:</strong></td>
                                          </tr>
                                          <tr>
                                            <td valign="top"><select name="allow_who" id="allow_who">
                                              <option value="all" <?php if ($photo['allow_who']=='all') { echo 'selected'; } ?>><? echo $_LANG['EVERYBODY']; ?></option>
                                              <option value="registered" <?php if ($photo['allow_who']=='registered') { echo 'selected'; } ?>><? echo $_LANG['REGISTERED']; ?></option>
                                              <option value="friends" <?php if ($photo['allow_who']=='friends') { echo 'selected'; } ?>><? echo $_LANG['MY_FRIENDS']; ?></option>
                                            </select></td>
                                          </tr>
                                          <tr>
                                            <td valign="top">
                                              <input style="margin-top:10px;font-size:18px" type="submit" name="save" value="<? echo $_LANG['SAVE']; ?>" />
										      <input style="margin-top:10px;font-size:18px" type="button" name="cancel" value="<? echo $_LANG['CANCEL']; ?>" onclick="window.history.go(-1)"/>
                                            </td>
                                          </tr>
                                        </table></td>
                                      </tr>
				...
				if(!isset($_POST['gosend'])){		
					...
					echo '<td width="200" height="200" valign="top">
							...
							<div style="padding:5px;width:100%">
								 ÓÏÛ: '.usrLink($usr['nickname'], $usr['id'], $menuid).'
							</div>
						 </td>';						 
					echo '<td valign="top">';
...
if ($do=='files'){
	...
	if ($inDB->num_rows($result)>0){
			...
			if ($inDB->num_rows($result)){ //if files exists
				...
				echo '<table width="100%" cellspacing="0" cellpadding="6" style="border:solid 1px gray">';
					echo '<tr>';
						echo '<td class="usr_files_head" width="20" align="center">#</td>';
						echo '<td class="usr_files_head" width="150">'.$_LANG['FILE_NAME'].' '.($orderby=='filename'?'&darr;':'').'</td>';
                        echo '<td class="usr_files_head" width="150">'.$_LANG['FILE_DESC'].' '.($orderby=='filedesc'?'&darr;':'').'</td>';
						if ($inUser->id==$id){
						...
						if ($inUser->id==$id){
							echo '<td class="usr_files_head" width="32">&nbsp;</td>';
						}
					echo '</tr>';
				$rownum = 0;
				...
				while($file = $inDB->fetch_assoc($result)){
						...
						echo '<tr>';
							...
							echo '<td class="'.$class.'" width="150" valign="top">'.$inCore->fileIcon($file['filename']).'<a href="'.$filelink.'"> '.$file['filename'].'</a><div class="usr_files_link">'.$filelink.'</div></td>';
                            echo '<td class="'.$class.'" width="150" valign="top"><div>'.$file['description'].'</div></td>';
         					if ($inUser->id==$id){
							...
							if ($inUser->id==$id){
                                ...
                                echo '<a href="/users/'.$menuid.'/'.$id.'/editdescrfile'.$file['id'].'.html"><img src="/admin/images/actions/edit.gif" border="0" alt="'.$_LANG['EDIT_FILE_DESC'].'"/></a>';
...
if ($do=='editdescrfile'){
    ...
    if (usrCheckAuth() && (@$inUser->id==$id || $inCore->userIsAdmin($inUser->id))){
        if (!isset($_POST['goeditdescrfile'])){
            $sql = "SELECT * FROM cms_users WHERE id = $id";
            $result = $inDB->query($sql) ;
            if ($inDB->num_rows($result)){
                $inPage->backButton(false);
                ...
                if ($inDB->num_rows($result)){
                    ...
                    $inPage->addPathway($usr['nickname'], cmsUser::getProfileURL($usr['login']));
                    $inPage->addPathway($_LANG['TITLE_FILE'], '/users/'.$menuid.'/'.$id.'/files.html');
                    $inPage->addPathway($_LANG['EDIT_DESC_FILE'], $_SERVER['REQUEST_URI']);
                    echo '<div class="con_heading">'.$_LANG['EDIT_DESC_FILE'].'</div>';
                    echo '<div><form action="'.$_SERVER['REQUEST_URI'].'" method="POST"><p>'.$_LANG['FILE'].': <b>'.$file['filename'].'</b><p>
                            ŒÔËÒ‡ÌËÂ: <input type=text name="newdescription" value="'.$file['description'].'" size=80 /></p><p>
                            <input style="font-size:24px; width:150px" type="submit" name="goeditdescrfile" value="'.$_LANG['SAVE'].'" />
                            <input style="font-size:24px; width:150px" type="button" name="cancel" value="'.$_LANG['CANCEL'].'" onclick="window.history.go(-1)" />
                            <input type=hidden name="fileid" value="'.$fileid.'" />
                         </p></form>';
                } else { usrAccessDenied(); }
            }
        } else {
            $inDB->query("UPDATE cms_user_files SET description = '".$_POST['newdescription']."' WHERE id=".$_REQUEST['fileid']) ;
