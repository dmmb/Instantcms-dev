...
{if $total_no_pub}
<p class="usr_photos_add_limit">{$LANG.NO_PUBLISHED_PHOTO}: <a href="/users/{$user.login}/photos/submit">{$total_no_pub|spellcount:$LANG.PHOTO:$LANG.PHOTO2:$LANG.PHOTO10}</a></p>
{/if}
{if !$stop_photo}
	{if $uload_type == 'multi'}
            ...
            <form id="usr_photos_upload_form" action="" method="post" enctype="multipart/form-data">
                ...
                {/if}
                    ...
                    <div id="divStatus" style="display:none">
                        ...
                        <a href="/users/{$user.login}/photos/submit" id="continue">{$LANG.CONTINUE}</a>
                    </div>
