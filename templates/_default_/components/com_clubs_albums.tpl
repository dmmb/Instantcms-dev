            ...
            {foreach key=key item=album from=$albums}
                <li id="{$album.id}">
                    <div class="usr_album_thumb">
                        <a href="/photos/{$album.id}" title="{$album.title}">
                            <img src="/images/photos/small/{$album.file}" width="64" height="64" border="0" alt="{$album.title}" />
                        </a>
                    </div>
                    <div class="usr_album">
                        <div class="link">
                            ...
                            {if $is_admin || $is_moder}
                            	&nbsp;<a class="delete" title="{$LANG.DEL_ALBUMS}" href="javascript:void(0)" onclick="javascript:deleteAlbum({$album.id}, '{$album.title}', {$club_id})">X</a>
                            {/if}
                        </div>
