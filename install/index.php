	...
	if (isset($_POST['install'])){
		...
		if(!$msg){
			...
			if(!$msg){
				...
				if ($_REQUEST['demodata']==1){
					$sql_file = 'sqldumpdemo.sql';
				} else {
					$sql_file = 'sqldumpempty.sql';
				}
					...
					$sql = "UPDATE {$db_prefix}_users SET password = md5('{$admin_password}'),
											 	 login = '{$admin_login}'
											 WHERE id = 1";
					mysql_query($sql);
...
function installCheckFolders(){
	$folders = array();
	$folders[] = '/images/users/avatars';
	$folders[] = '/images/users/photos';
	$folders[] = '/upload';
	$folders[] = '/includes';
	$folders[] = '/backups';
    $folders[] = '/cache';
	$folders[] = '/templates/_default_/modules/modules_c';
	$folders[] = '/templates/_default_/components/components_c';
	$folders[] = '/templates/_default_/plugins/plugins_c';
	...
	<tr><td>
		...
		<form class="wizard" action="#" method="post" >
			...
			<div id="start" class="wizardpage">
				...
				<p>Как установить систему на локальный компьютер для тестирования, читайте в <a href="http://www.instantcms.ru/content/40/read33.html" target="_blank">инструкции</a> на официальном сайте.</p>
		...
		<div id="footer">
			...
			<a href="http://www.instantcms.ru/content/40/read33.html" style="margin-left:150px" target="_blank">Инструкция по установке</a> | <a href="mailto:info@instantcms.ru" title="Наша почта">Написать нам</a>
		</div>
