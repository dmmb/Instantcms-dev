...
echo "Deleting Smarty cache...";
rm -f $TO_DIR/templates/_default_/components/components_c/*;
rm -f $TO_DIR/templates/_default_/modules/modules_c/*;
rm -f $TO_DIR/templates/_default_/plugins/plugins_c/*;
...
find $TO_DIR -maxdepth 1 -mindepth 1 -type f -not -name .htaccess -not -name readme.txt -not -name version_log.txt -not -name license.txt -not -name license.rus.win.txt -not -name license.rus.utf.txt -not -name index.php -not -name $DISTR_NAME -print0 | xargs -0 rm -f;
echo "";
