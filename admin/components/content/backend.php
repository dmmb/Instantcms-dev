...
/*********************************************************************************************/
	...
	if($opt=='saveconfig'){	
		$cfg = array();
		$cfg['showtitle']   = $inCore->request('showtitle', 'int', 0);
...
<form action="index.php?view=components&do=config&id=<?php echo $_REQUEST['id'];?>" method="post" name="optform" target="_self" id="form1">
    ...
    <table width="550" border="0" cellpadding="10" cellspacing="0" class="proptable">
        <tr>
            ...
            <td width="110">
                <input name="showtitle" type="radio" value="1" <?php if (@$cfg['showtitle']) { echo 'checked="checked"'; } ?>/> Да
                <input name="showtitle" type="radio" value="0" <?php if (@!$cfg['showtitle']) { echo 'checked="checked"'; } ?>/> Нет
            </td>
        </tr>
        <tr>
