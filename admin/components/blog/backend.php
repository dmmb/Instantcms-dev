...
/*********************************************************************************************/
    ...
    if ($opt=='config'){
        ...
        $toolmenu[2]['link'] = '?view=components&do=config&link=blog&opt=list_blogs';
        cpToolMenu($toolmenu);
    ...
    if ($opt=='list_blogs'){
		...
		$toolmenu[1]['link'] = "javascript:checkSel('?view=components&do=config&link=blog&opt=edit_blog&multiple=1');";

		$toolmenu[2]['icon'] = 'delete.gif';
		$toolmenu[2]['title'] = 'Удалить выбранные';
		$toolmenu[2]['link'] = "javascript:checkSel('?view=components&do=config&link=blog&opt=delete_blog&multiple=1');";

        $toolmenu[3]['icon'] = 'config.gif';
        $toolmenu[3]['title'] = 'Настройки компонента';
        $toolmenu[3]['link'] = '?view=components&do=config&link=blog&opt=config';
        cpToolMenu($toolmenu);
		...
		$fields[2]['title']  = 'Название';	$fields[2]['field'] = 'title';			$fields[2]['width'] = '';		$fields[2]['link'] = '?view=components&do=config&link=blog&opt=edit_blog&item_id=%id%';
		$fields[2]['filter'] = 15;
		...
		$actions[1]['link']  = '?view=components&do=config&link=blog&opt=edit_blog&item_id=%id%';

		$actions[2]['title'] = 'Удалить';
		$actions[2]['icon']  = 'delete.gif';
		$actions[2]['confirm'] = 'Удалить блог?';
		$actions[2]['link']  = '?view=components&do=config&link=blog&opt=delete_blog&item_id=%id%';
	...
	if ($opt == 'delete_blog'){
		...
		header('location:?view=components&do=config&link=blog&opt=list_blogs');
	}
	...
	if ($opt == 'update_blog'){
		if($inCore->request('item_id', 'int', 0)) {
			...
			if (!isset($_SESSION['editlist']) || @sizeof($_SESSION['editlist'])==0){
				...
				header('location:?view=components&do=config&link=blog&opt=edit_blog');
			}
		}
	}
...
?>
<form action="index.php?view=components&do=config&link=blog&opt=update_blog&item_id=<?php echo $mod['id']; ?>" method="post" name="optform" target="_self" id="form1">
    <table width="609" border="0" cellpadding="10" cellspacing="0" class="proptable">
        <tr>
    ...
    <p>
        ...
        <input name="save" type="submit" id="save" value="Сохранить" />
        <input name="back" type="button" id="back" value="Отмена" onclick="window.location.href='index.php?view=components&do=config&link=blog&opt=list_blogs';"/>
    </p>
</form>
<?php } ?>