					...
					if ($is_actions){
						...
						foreach($_actions as $key => $value){
							...
							if ($print){
								...
								$link = str_replace('%id%', $item['id'], $_actions[$key]['link']); 
...
echo '<table width="600" border="0" cellspacing="0" cellpadding="8" class="proptable"><tr><td>';
	echo '<table width="580" border="0" cellspacing="0" cellpadding="2">';
	echo '<tr>';
		echo '<td width="10">';
	...
	echo '<tr>';
		...
		echo '<div id="material" style="float:left"> 
				<strong>Материал:</strong> <select name="m" style="border:solid 1px black">'.$inCore->getListItems('cms_content').'</select>
			  </div>';
