...
function applet_users(){
	...
	if ($do == 'list'){
		...
		$actions[1]['link']  = '?view=users&do=edit&id=%id%';
		
		$actions[2]['title'] = 'Забанить';
		$actions[2]['icon']  = 'ban.gif';
		$actions[2]['link']  = '/admin/index.php?view=userbanlist&do=add&to=%id%';	

		$actions[3]['title'] = 'Удалить';
		$actions[3]['icon']  = 'delete.gif';
		$actions[3]['confirm'] = 'Удалить пользователя?';
		$actions[3]['link']  = '?view=users&do=delete&id=%id%';
	...
	?>
      <form action="index.php?view=users" method="post" enctype="multipart/form-data" name="addform" id="addform">
        <table width="600" border="0" cellpadding="0" cellspacing="10" class="proptable">
          <tr>
            <td width="177" valign="middle"><strong>Логин: </strong></td>
          ...
          <tr>
            ...
            <td valign="middle">
			<select name="group_id" id="group_id" style="width:220px">
                <?php
			...
			if ($do=='edit'){
				if (isset($mod['group_id'])) { 
					echo '<a href="?view=usergroups&do=edit&id='.$mod['group_id'].'"><img src="images/icons/edit.png" border="0" alt="Редактировать группу"/></a>'; 
