...
function createMenuItem($menu, $id, $title){
	...
	$sql = "UPDATE cms_menu 
			SET menu='$menu', 
				title='$title', 
				link='/index.php?view=content&do=read&id=$id', 
				linktype='content', 
				linkid='$id', 
				target='_self', 
				published='1', 
...
function applet_content(){
	...
	if ($do == 'list'){
		...
		$toolmenu[7]['icon'] = 'arhive.gif';
		$toolmenu[7]['title'] = 'Архив материалов';
		$toolmenu[7]['link'] = "?view=arhive";
	...
	if ($do == 'update'){
		if(isset($_REQUEST['id'])) {
            ...
            $article['meta_desc']      = $inCore->request('meta_desc', 'str');
			$article['meta_keys']      = $inCore->request('meta_keys', 'str');
            
			if ($inCore->inRequest('autokeys')){
				$article['meta_desc']  = $inCore->getKeywords($article['content']);
            ...
            <tr>
                ...
                <td valign="top">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" border="0" class="checklist">
                        <tr>
                            <td width="20"><input type="checkbox" name="autokeys" id="autokeys" <?php if ($do=='add'){ ?>checked="checked"<?php } ?> value="1"/></td>
                            <td><label for="autokeys"><strong>Автоматически сгенерировать ключевые слова и описание</strong></label></td>
                        </tr>
