...
/*********************************************************************************************/
	...
	editorCheckAuth(); //CHECK AUTHENTICATION
...
if ($do=='view'){
	...
	$sql = "SELECT *, DATE_FORMAT(pubdate, '%d-%m-%Y') as fpubdate, DATE_FORMAT(pubdate, '%H:%i') as fpubtime
			FROM cms_content
		...
		while($con = mysql_fetch_assoc($result)){
			...
			if($inCore->checkUserAccess('material', $con['id'])){
				echo '<tr>';
					...
					echo '<td style="'.$style.'" width="25" valign="top"><a href="/content/0/read'.$con['id'].'.html#c" title="Комментарии">'.$inCore->getCommentsCount('article', $con['id']).'</a></td>';			
...
if ($do == 'submitdoc'){
	...
	$sql = "INSERT INTO cms_content (category_id, user_id, pubdate, enddate, is_end, title, description, content, published, hits, meta_desc, meta_keys, showtitle, showdate, showlatest, ordering, comments, seolink, canrate)
			VALUES ($category_id, '$user_id', NOW(), '$enddate', $is_end, '$title', '$description', '$content', $published, 0, '$meta_desc', '$meta_keys', '1', '$showdate', '$showlatest', LAST_INSERT_ID()+1, $comments, '', 1)";
	dbQuery($sql);
...
if ($do == 'deletedoc'){
	if (isset($id)){	
		if (isset($_GET['confirm'])){
			dbQuery("DELETE FROM cms_content WHERE id = $id") ;
